...
- Remove `NO_RELATIVISTIC_ECCENTRICITY` from the intended architecture and top-level project narrative.
- Keep the repo framed as `CSL` versus `VSL`, not a three-mode comparison.
...
- Update diagnostics wording where needed so CSL remains the operational benchmark without sounding like the only meaningful model.
