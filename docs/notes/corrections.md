...
This document compares items present on the CSL side of the current codebase with the corresponding VSL ballistic/emission status in the current code. It is intentionally limited to implemented code paths, not every correction used in full GPS literature.

| CSL-side item in current code | VSL status in current code |
| --- | --- |
| Relativistic eccentricity clock term in `src/csl/clock.py`: `F * e * sqrt(A) * sin(E)` | Removed from `src/vsl/clock.py`. VSL keeps eccentricity in the Keplerian orbit and ballistic propagation and applies no eccentricity-dependent clock correction. Gravity enters only through the corpuscle light-speed model in `src/vsl/corrections.py`. |
...
- VSL treats the emitted signal as a Newtonian corpuscle: `effective_light_speed_mps(...)` computes the path-averaged light speed between the satellite and receiver radii from conservation of mechanical energy, and falling light speeds up while rising light slows down. No `F_RELATIVISTIC`, no photon-frequency correction, and no gravitational clock-rate correction appear anywhere in the VSL code.
...
- Removing the half-size periodic term (A to B) degraded fit quality by roughly `1.7 m` and `1.2 m` of RMS delta.
...
- Configurations A and C are numerically near-equivalent because the signal shift equals half of the standard periodic eccentricity correction, and a pseudorange residual cannot tell whether a term of that size was applied to the satellite clock or to the received signal.
- Configuration C was subsequently dropped: its frequency-shift interpretation was removed, and no periodic term remains in the current VSL model.
- Configuration D treats light as a Newtonian corpuscle whose speed follows conservation of mechanical energy between the satellite and receiver radii. The corpuscle speed effect amounts to only millimetres of range, so results return to near configuration B.
