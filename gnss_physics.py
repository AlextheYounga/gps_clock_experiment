...
def calculate_clock_correction(
    ...
    *,
    ...
    tx_including_week_s = (
        receiver_gps_week_at_transmission * SECONDS_IN_WEEK
        + receiver_gps_tow_at_transmission_s
    )

    tc_s = tx_including_week_s - (ephemeris.week * SECONDS_IN_WEEK + ephemeris.toc)
    tc_s = fix_week_rollover(tc_s)

    init_sat_clock_correction_s = (
        ephemeris.af0
        + ephemeris.af1 * tc_s
        + ephemeris.af2 * tc_s * tc_s
        - ephemeris.tgd
    )
    ...
    while True:
        tk_s = tx_including_week_s - (
            ephemeris.week * SECONDS_IN_WEEK + ephemeris.toe + sat_clock_correction_s
        )
        ...
        while True:
            ...
            eccentric_anomaly_rad = mean_anomaly_rad + ephemeris.e * math.sin(
                eccentric_anomaly_rad
            )
            eccentric_counter += 1
            if eccentric_counter > MAX_ITERATIONS:
                raise RuntimeError(
                    f"Kepler eccentric anomaly did not converge in {MAX_ITERATIONS} iterations"
                )
        ...
        new_sat_clock_correction_s = (
            init_sat_clock_correction_s + relativistic_correction_s
        )
        ...
        if sat_clock_corrections_counter > MAX_ITERATIONS:
            raise RuntimeError(
                f"Satellite clock correction did not converge in {MAX_ITERATIONS} iterations"
            )
...
def calculate_corrected_transmit_tow_and_week(
    ...
    *,
    ...
    receiver_gps_tow_at_tx_s = (
        receiver_gps_tow_at_reception_s - pseudorange_m / SPEED_OF_LIGHT_MPS
    )
...
def _calculate_satellite_position(
    ...
    *,
    ...
    radius_correction_m = ephemeris.crc * math.cos(
        2.0 * argument_of_latitude_rad
    ) + ephemeris.crs * math.sin(2.0 * argument_of_latitude_rad)
    arg_lat_correction_rad = ephemeris.cuc * math.cos(
        2.0 * argument_of_latitude_rad
    ) + ephemeris.cus * math.sin(2.0 * argument_of_latitude_rad)
    inclination_correction_rad = ephemeris.cic * math.cos(
        2.0 * argument_of_latitude_rad
    ) + ephemeris.cis * math.sin(2.0 * argument_of_latitude_rad)
    ...
    sat_x_m = x_orb_m * math.cos(omega_k_rad) - y_orb_m * math.cos(
        inclination_rad
    ) * math.sin(omega_k_rad)
    sat_y_m = x_orb_m * math.sin(omega_k_rad) + y_orb_m * math.cos(
        inclination_rad
    ) * math.cos(omega_k_rad)
