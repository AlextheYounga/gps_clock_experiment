"""Parsing helpers for GNSS log and navigation files."""
...
DEFAULT_MIN_CN0_DBHZ = 18.0
DEFAULT_CONSTELLATION_TYPE_GPS = 1
DEFAULT_TOW_DECODED_STATE_BIT = 3
...
def parse_rinex_nav_file(path: str | Path) -> dict[int, list[Ephemeris]]:
    """Parse a RINEX navigation file into ephemerides grouped by PRN."""
    file_path = Path(path)
...
def parse_gnss_logger_file(
    ...
    *,
    gps_week: int,
    min_cn0_dbhz: float = DEFAULT_MIN_CN0_DBHZ,
    constellation_type_gps: int = DEFAULT_CONSTELLATION_TYPE_GPS,
    tow_decoded_state_bit: int = DEFAULT_TOW_DECODED_STATE_BIT,
    ...
    epochs = _build_epochs(epochs_raw, gps_week)

    return epochs, fixes


def iter_all_ephemerides(nav_by_prn: dict[int, list[Ephemeris]]) -> Iterable[Ephemeris]:
    """Yield every ephemeris from a PRN-indexed navigation mapping."""
